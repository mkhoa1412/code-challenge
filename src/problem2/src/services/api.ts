...
export const fetchTokenPrices = async (): Promise<TokenPrice[]> => {
  try {
    const response = await apiClient.get<TokenPrice[]>('/token-prices')
...
export const calculateExchangeRate = (
  ...
  const getLatestPrice = (currency: string): number => {
    const currencyPrices = prices.filter(p => p.currency === currency)
    if (currencyPrices.length === 0) return 1
    
    const latest = currencyPrices.sort((a, b) => 
...
export const getAvailableCurrencies = (prices: TokenPrice[]): string[] => {
